    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 md:px-6">
        <header className="mx-auto max-w-3xl text-center" data-reveal>
          <span className="inline-flex rounded-full border border-slate-200 bg-[#eef1f5] px-5 py-2 text-xs font-semibold tracking-[0.22em] text-[#1a3a6c]">
            USER STORIES
          </span>
          <h2 className="mt-5 text-4xl font-semibold leading-tight text-[#1a3a6c] md:text-5xl lg:text-[3.5rem]">
            Real Journeys,{" "}
            <em className="font-semibold text-red-600">Beautiful Experiences</em>
          </h2>
          <p className="mx-auto mt-5 max-w-2xl text-sm leading-7 text-slate-600 md:text-base">
            ...
            through Krishna Coming.
          </p>
        </header>

        <div
          className="stories-track mt-12 flex snap-x snap-mandatory gap-5 overflow-x-auto pb-6 sm:grid sm:grid-cols-2 sm:overflow-visible sm:pb-0 lg:grid-cols-3 xl:flex xl:flex-nowrap xl:overflow-x-auto"
          data-reveal
        >
          {storiesData.map((story, index) => (
            <article
              key={story.name}
              className="story-card group min-w-[86%] snap-center overflow-hidden sm:min-w-0 xl:min-w-[350px] shrink-0"
            >
              <button
                type="button"
              ...
              >
                ...
                />
                <span className="absolute inset-0 bg-gradient-to-t from-[#1a3a6c]/60 via-transparent to-transparent" />
                ...
                <span className="absolute -bottom-5 right-5 grid h-12 w-12 place-items-center rounded-2xl bg-[#1a3a6c] text-3xl text-white shadow-lg">
                  “
                </span>
              </button>
              <div className="p-6 pt-7">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h3 className="text-2xl font-semibold text-[#1a3a6c]">
                      {story.name}
                    </h3>
                    <p className="mt-1 text-xs font-medium uppercase tracking-[0.15em] text-red-600">
                      Verified family
                    </p>
                  </div>
                  <div
                ...
                <p className="mt-5 min-h-20 text-sm leading-7 text-slate-600">“{story.quote}”</p>
                <div className="mt-5 grid grid-cols-3 border-t border-slate-200 pt-5">
                  {benefits.map(({ icon: Icon, label }) => (
                    <div key={label} className="flex flex-col items-center gap-2 px-1 text-center">
                      <span className="grid h-8 w-8 place-items-center rounded-full bg-[#eef1f5] text-[#1a3a6c]">
                        <Icon />
                      </span>
        ...
        <div
          className="mt-12 grid overflow-hidden rounded-[28px] border border-slate-200 bg-white/80 px-5 py-6 shadow-lg backdrop-blur-xl sm:grid-cols-2 lg:grid-cols-4"
          data-reveal
        >
          {stats.map(({ icon: Icon, value, label }, index) => (
            <div
              key={label}
              className={`flex items-center justify-center gap-3 px-4 py-3 ${index ? "lg:border-l lg:border-slate-200" : ""}`}
            >
              <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-[#eef1f5] text-xl text-[#1a3a6c]">
                <Icon />
              </span>
              <div>
                <strong className="block text-lg font-semibold text-[#1a3a6c]">{value}</strong>
      ...
      {activeVideo && (
        <div
          ...
          className="fixed inset-0 z-[70] grid place-items-center bg-[#1a3a6c]/85 p-4 backdrop-blur-md"
        >
...
function Decorations() {
  return (
    <div aria-hidden="true" className="pointer-events-none absolute inset-0">
      ...
      <span className="story-float absolute right-[12%] top-[22%] text-xl text-[#1a3a6c]/20">
        ✦
      </span>
      <span className="story-float absolute bottom-[22%] left-[6%] text-lg text-[#F4C27A]/70 [animation-delay:1.2s]">
        ✧
      </span>
      <span className="absolute right-[5%] top-1/2 h-24 w-24 rounded-full bg-[#1a3a6c]/10 blur-2xl" />
